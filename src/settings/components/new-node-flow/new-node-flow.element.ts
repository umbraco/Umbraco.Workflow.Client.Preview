...
export class WorkflowNewNodeFlowElement extends UmbLitElement {
  ...
  constructor() {
    ...
    this.consumeContext(WORKFLOW_SETTINGS_WORKSPACE_CONTEXT, (workspaceContext) => {
      if (!workspaceContext) return;

      this.#workspaceContext = workspaceContext;

      this.observe(this.#workspaceContext.newNodeApprovalFlow, (newNodeApprovalFlow) => {
        this.value = (newNodeApprovalFlow?.value ?? []) as Array<ApprovalGroupDetailPermissionConfigModel>;
        this.documentUnique = newNodeApprovalFlow?.config.find((x) => x.alias === "nodeKey")?.value as string;
      });
    });
  }
  ...
  render() {
    return html`<workflow-approval-group-input
      .config=${{
        ...
        configureThreshold: this.#workspaceContext?.getConfigureApprovalThreshold(),
      }}
