...
> {
  ...
  connectedCallback() {
    ...
    this.consumeContext(UMB_APP_LANGUAGE_CONTEXT, (context) => {
      ...
      this.observe(context.languages, (languages) => {
        ...
        if (this.current && !this.current.culture) {
          this.current.culture = this._languages.at(0)?.unique;
        }
      });
    });
  }
  ...
  #getExistingIndex() {
    return this.configItems.findIndex(
      (x) => x.culture === this.current?.culture
    );
  }
  ...
  #handleLanguageChange(e: UUIInputEvent) {
    const idx = this.configItems.findIndex((x) => x.culture === e.target.value);
    ...
    this.current = {
      culture: e.target.value as string,
      groups: [],
      excluded: false,
      externalReviewers: "",
  ...
  async #onApprovalGroupsUpdated(e: CustomEvent) {
    ...
    const groups =
      target.selectedPermissions?.map((p) => ({
        icon: p.icon ?? "users",
        name: p.name,
    ...
    return html`<uui-box .headline=${this.localize.term("general_language")}>
      <uui-select
        ...
        .options=${this._languages.map((l) => ({
          name: l.name!,
          value: l.unique!,
          selected: l.unique === this.current?.culture,
        })) ?? []}
  ...
  #renderGroupBox() {
    return html`<uui-box
      .headline=${this.localize.term("workflow_contentReviews_reviewGroup")}
    >
  ...
  #renderExternalReviewersBox() {
    return html`<uui-box
      .headline=${this.localize.term(
        "workflow_contentReviews_externalReviewers"
      )}
    >
      ${this.localize.term(
        "workflow_contentReviews_externalReviewersDescription"
      )}
      <umb-property-layout
